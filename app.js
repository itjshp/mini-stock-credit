...
function renderHistories() {
  $("purchaseHistory").innerHTML = state.purchases.slice(0, 30).map(p => `
    <div class="list-item">
      <div>
        ...
        <small>${p.date} • จำนวน ${money(p.qty)} • ทุน ${money(p.unitCost)} ${p.extraCost ? "• ค่าส่ง " + money(p.extraCost) : ""}</small>
        ${p.note ? `<small>หมายเหตุ: ${p.note}</small>` : ""}
      </div>
      <div class="row-actions">
        <div class="money">${money(p.totalCost)}</div>
        <button class="small-btn small-edit" onclick="editPurchase('${p.id}')">แก้ไข</button>
  ...
  $("saleHistory").innerHTML = state.sales.slice(0, 30).map(s => `
    <div class="list-item">
      <div>
        <strong>${productName(s.productId)}</strong> ${paymentBadge(s.paymentType)}
        <small>${s.date} • ${customerName(s.customerId)} • จำนวน ${money(s.qty)} • ราคา ${money(s.unitPrice)}</small>
        ${s.note ? `<small>หมายเหตุ: ${s.note}</small>` : ""}
      </div>
      <div class="row-actions">
        <div>
        ...
        <button class="small-btn small-edit" onclick="editSale('${s.id}')">แก้ไข</button>
...
function resetPurchaseForm() {
  $("purchaseForm").reset(); $("purchaseId").value = ""; $("purchaseDate").value = today(); $("purchaseExtraCost").value = 0;
  $("purchaseSubmitBtn").textContent = "บันทึกซื้อเข้า";
  $("purchaseEditBanner").classList.add("hidden"); $("cancelPurchaseEdit").classList.add("hidden");
}
function resetSaleForm() {
  $("saleForm").reset(); $("saleId").value = ""; $("saleDate").value = today(); $("salePaidAmount").value = 0;
  $("saleSubmitBtn").textContent = "บันทึกขาย";
  $("saleEditBanner").classList.add("hidden"); $("cancelSaleEdit").classList.add("hidden");
  updateSalePreview();
}
function resetPaymentForm() {
  $("paymentForm").reset(); $("paymentId").value = ""; $("paymentDate").value = today();
  $("paymentSubmitBtn").textContent = "บันทึกรับชำระ";
  $("paymentEditBanner").classList.add("hidden"); $("cancelPaymentEdit").classList.add("hidden");
}
...
$("purchaseForm").addEventListener("submit", async (e) => {
  ...
  resetPurchaseForm();
  showToast(existing.id ? "อัปเดตรายการซื้อเข้าแล้ว" : "บันทึกซื้อเข้าแล้ว");
  await refreshState();
});
...
window.editPurchase = (id) => {
  ...
  $("purchaseDate").value = p.date || today();
  $("purchaseProduct").value = p.productId || "";
  $("purchaseQty").value = p.qty || "";
  $("purchaseUnitCost").value = p.unitCost || "";
  $("purchaseExtraCost").value = p.extraCost || 0;
  $("purchaseNote").value = p.note || "";
  $("purchaseSubmitBtn").textContent = "อัปเดตซื้อเข้า";
  $("purchaseEditBanner").classList.remove("hidden"); $("cancelPurchaseEdit").classList.remove("hidden");
  switchTab("purchase");
};
...
$("saleForm").addEventListener("submit", async (e) => {
  ...
  const product = state.products.find(p => p.id === $("saleProduct").value);
  if (!product) return alert("กรุณาเลือกสินค้า");
  const id = $("saleId").value || uid();
  const existing = state.sales.find(s => s.id === id) || {};
  const qty = Number($("saleQty").value || 0);

  const oldQtySameProduct = existing.productId === product.id ? Number(existing.qty || 0) : 0;
  const available = Number(product.stockQty || 0) + oldQtySameProduct;
  if (qty > available) return alert(`สต็อกไม่พอ เหลือที่ขายได้ ${money(available)} ${product.unit || ""}`);
  ...
  await rebuildInventoryFromTransactions();
  resetSaleForm();
  showToast(existing.id ? "อัปเดตรายการขายแล้ว" : "บันทึกขายแล้ว");
  await refreshState();
});
...
window.editSale = (id) => {
  ...
  if (!s) return;
  $("saleId").value = s.id;
  $("saleDate").value = s.date || today();
  $("saleCustomer").value = s.customerId || "";
  $("saleProduct").value = s.productId || "";
  $("saleQty").value = s.qty || "";
  $("saleUnitPrice").value = s.unitPrice || "";
  $("salePaymentType").value = s.paymentType || "cash";
  $("salePaidAmount").value = s.paidAmount || 0;
  $("saleNote").value = s.note || "";
  $("saleSubmitBtn").textContent = "อัปเดตรายการขาย";
  $("saleEditBanner").classList.remove("hidden"); $("cancelSaleEdit").classList.remove("hidden");
  updateSalePreview();
  switchTab("sale");
};
...
$("paymentForm").addEventListener("submit", async (e) => {
  ...
  resetPaymentForm();
  showToast(existing.id ? "อัปเดตรับชำระแล้ว" : "บันทึกรับชำระแล้ว");
  await refreshState();
});
...
window.editPayment = (id) => {
  ...
  $("paymentSubmitBtn").textContent = "อัปเดตรับชำระ";
  $("paymentEditBanner").classList.remove("hidden"); $("cancelPaymentEdit").classList.remove("hidden");
  switchTab("payments");
};
...
$("exportBackupBtn").addEventListener("click", () => {
  const data = { app: "Mini Stock Credit", version: "0.3", exportedAt: new Date().toISOString(), ...state };
